...
def visualize_tree(estimator, X, y, ax: plt.Axes, boundaries=True,
                   xlim=None, ylim=None):
    estimator.fit(X, y)
...
def plot_kmeans_interactive(min_clusters=2, max_clusters=6):
    from ipywidgets import interact
    ...
    with warnings.catch_warnings():
        ...
        def _kmeans_step(frame=0, n_clusters=4):
            ...
            centers = rng.standard_normal((n_clusters, 2))
            ...
            for i in range(nsteps + 1):
                ...
                if i < nsteps or frame % 3 > 1:
                    # for the second cluster, no labels match... this causes mean of empty slice issue
                    centers = np.array([X[labels == j].mean(0) if X[labels == j].any() else [np.nan] * 2
            ...
            ax.scatter(old_centers[:, 0], old_centers[:, 1], marker='o',
            ...
            if frame % 3 == 1:
                ax.text(3.8, 9.5, "1. Reassign points to nearest centroid",
                        ha='right', va='top', size=14)
            elif frame % 3 == 2:
                ax.text(3.8, 9.5, "2. Update centroids to cluster means",
                        ha='right', va='top', size=14)
